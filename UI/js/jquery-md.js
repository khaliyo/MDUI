...
//*********************StringBuilder*******************************
md.system.string = {};
md.system.string.StringBuilder = function () { this._string_ = new Array(); }
md.system.string.StringBuilder.prototype.Append = function (str) { this._string_.push(str); }
md.system.string.StringBuilder.prototype.toString = function () { return this._string_.join("");}
...
/*alert弹层*/
md.system.alert = function (settings) {
    $("body").Alert(settings);
};

/*分页插件*/
md.system.pager = function (settings) {
    if (settings.elementID) {
        $(settings.elementID).Pager(settings);
    }
};

/*下拉框插件*/
md.system.select = function (settings) {
    if (settings.elementID) {
        $(settings.elementID).MDSelect(settings);
    }
};
...
md.system.tip = function (settings) {
    if (settings.elementID) {
        $(settings.elementID).poshytip("destroy");
        $(settings.elementID).poshytip(settings);
    }
};
...
document.write('<link rel="stylesheet" href="' + md.api.options.fileRoot + 'css/md.css" type="text/css" />');


md.api.getQueryString = function (key) {
    if ($("script[src^='" + md.api.options.fileRoot + "js/jquery-md.js']").length > 0) {
        var url = $("script[src^='" + md.api.options.fileRoot + "js/jquery-md.js']").attr("src");
...
md.api.getAppBaseFrame = function (settings) {
    md.api.options.user.userID = md.api.getQueryString("u");
    md.api.options.project.projectID = md.api.getQueryString("p");

    $.extend(md.api.options.container, settings);
    if (md.api.options.container.right != "")
        $(md.api.options.container.right).hide();

    if (md.api.options.user.userID && md.api.options.user.userID != "") {
        var url = md.api.options.apiUrl + "md/v2/userinfo?userID=" + md.api.options.user.userID + "&projectID=" + md.api.options.project.projectID + "&jsoncallback=?";
        $.ajax({
            url: url,
            dataType: "jsonp",
            success: function (data) {
                if (data) {
                    md.api.options.user = data.user;
                    md.api.options.project = data.project;
                    md.api.options.applications = data.applications;
                    md.api.options.host = data.project.md_url;

                    if (md.api.options.container.top != "")
                        md.api.getHeader(md.api.options.container.top);
                    if (md.api.options.container.left)
                        md.api.getLeft(md.api.options.container.left);
                    if (md.api.options.container.right != "")
                        $(md.api.options.container.right).show();
                    if (md.api.options.container.bootom != "")
                        md.api.getFooter(md.api.options.container.bootom);
                    //回调函数
                    md.api.options.container.callback();
                }
            }
        });
    }
...
md.api.getHeader = function (container) {
    var html = new md.system.string.StringBuilder();
    html.Append('<div id="mdTopBarContainer">');
    html.Append('<div id="mdTopBar" class="mdThemeBGColor3 mdThemeBorderColor1">');
    html.Append('<div class="topBarContent">');
    html.Append('<table cellpadding="0" cellspacing="0" border="0" width="100%" class="tabConetnt">');
    html.Append('<tr>');
    html.Append('<td width="3px">');
    html.Append('</td>');
    html.Append('<td align="center" class="homeTd" style="width: ' + md.api.options.container.customHomeWidth + 'px;">');
    if (md.api.options.container.customHome != "")
        html.Append(md.api.options.container.customHome);
    else {
        html.Append('<span class="homeBtn Hand" onclick="window.location.href=\'' + md.api.options.host + '\'">');
        html.Append('<span class="topBarIcon icon-nethome mdThemeColor5" title="返回首页"></span>');
        html.Append('</span>');
    }
    html.Append('</td>');
    html.Append('<td>');
    html.Append('<a href="#pageTop" class="InlineBlock" style="height:35px; width:100%" id="mdTopBarBackTop" ></a>');
    html.Append('</td>');
    html.Append('<td width="56" align="center">');
    html.Append('<div onclick="window.location.href=\'' + md.api.options.host + 'inbox\'">');
    html.Append('<span class="topBarIcon icon-inbox mdThemeColor5"></span>');
    html.Append('</div>');

    html.Append('</td>');
    html.Append('<td width="56" align="center">');
    html.Append('<div id="mdUserCenter">');
    html.Append('<i class="topBarIcon icon-charger mdThemeColor5"></i>');
    html.Append('</div>');
    html.Append('<div id="mdUserSetTip">');
    html.Append('<div id="mdUserSet">');
    html.Append('<ul class="userSetUL">');
    html.Append('<li><a href="' + md.api.options.host + 'myaccount/users">通讯录</a></li>');
    html.Append('<li><a href="' + md.api.options.host + 'myaccount/">个人中心</a></li>');
    html.Append('<li><a href="' + md.api.options.host + 'myaccount/updatepsd">密码设置</a></li>');
    html.Append('<li><a href="' + md.api.options.host + 'myaccount/settingnotice">通知设置</a></li>');
    html.Append('<li><a href="' + md.api.options.host + 'myaccount/lang">语言设置</a></li>');
    html.Append('</ul>');
    html.Append('<div><a href="' + ((md.api.options.container.logoutUrl == "") ? (md.api.options.host + "logout") : (md.api.options.container.logoutUrl)) + '" class="mdBtn mdBtn-enable" style="padding:5px 10px" >安全退出</a></div>');
    html.Append('</div>');
    html.Append('</div>');
    html.Append('</td>');
    html.Append('</tr>');
    ...
    if (!md.api.options.container.hideHeader) {
        html.Append('<div id="mdHeadContainer">');
        html.Append('<div id="mdHead">');
        html.Append('<table id="logoEvent" class="head_Logo" border="0" cellpadding="0" cellspacing="0">');
        html.Append('<tr>');
        html.Append('<td class="logoImg" style="width: 95px;">');
        html.Append('<a href="' + md.api.options.host + '"><img src="' + md.api.options.project.logo + '"></a>');
        html.Append('</td>');
        html.Append('<td width="45px">');
        html.Append('</td>');
        html.Append('</tr>');
        html.Append('</table>');
        html.Append('<div id="companyNameEvent" class="head_CompanyTitle">');
        html.Append('<div style="height:25px;"></div>');
        html.Append('<div class="txtCompanyName">' + md.api.options.project.name + '</div>');
        html.Append('</div>');
        html.Append('<div class="head_Links">');
        html.Append('<div class="subNav">');
        html.Append('<i class="icon-tool mdThemeColor3"></i><a href="' + md.api.options.host + 'admin/users" class="mdThemeColor3">网络管理</a>');
        html.Append('</div>');
        html.Append('<div class="mdClear"></div>');
        html.Append('</div>');
        html.Append('<div class="mdClear">');
        html.Append('</div>');
        html.Append('</div>');
        html.Append('</div>');
    }
    ...
    md.system.tip({
        elementID: "#mdUserCenter",
        className: 'tip-white',
        content: $("#mdUserSetTip").html(),
        showTimeout: 0,
        alignTo: 'target',
        alignX: 'center',
        offsetY: 8
...
md.api.getLeft = function (container) {
    var html = new md.system.string.StringBuilder();
    html.Append('<div id="mdLeftNav">');

    html.Append('<div class="menuItem activeItem">');
    html.Append('<div><div class="mdLeft">开放沟通</div><div class="mdLeft mdHidden arrowContainer"><i class="icon-arrow-up-border"></i></div><div class="mdClear"></div></div>');
    html.Append('</div>');
    ...
    html.Append('<li class="navItem"><i class="mdThemeColor4 icon-feed"></i><a href="' + md.api.options.host + 'feed#company">动态更新</a></li>');
    if (md.api.options.user.auto_followed == "0") {
        html.Append('<li class="navItem"><i class="mdThemeColor4 icon-follow"></i><a href="' + md.api.options.host + 'feed#followed">我关注的</a></li>');
    }
    html.Append('<li class="navItem"><i class="mdThemeColor4 icon-collect"></i><a href="' + md.api.options.host + 'feed#favorite">我收藏的</a></li>');
    html.Append('<li class="navItem"><i class="mdThemeColor4 icon-group"></i><a id="mdMyGroups" href="javascript:void(0);">我的群组<span class="groupArrow icon-arrow-down-border"></span></a></li>');
    html.Append('</ul>');
    html.Append('<ul id="mdUlGroups" class="ulGroups" style="display:none" >');
    if (md.api.options.user.groups) {
        for (var i = 0; i < md.api.options.user.groups.length; i++) {
            var item = md.api.options.user.groups[i];
            html.Append('<li><a href="' + md.api.options.host + 'group/feed?gID=' + item.id + '" title="' + item.name + '">');
            if (item.is_verified == "1")
                html.Append("<span class=\"mdLeft mdFont14 icon-official-group mdYellow\" style=\"margin-top:6px;margin-left:-19px\"  title=\"网络管理员创建的官方群组\"></span>");
            html.Append("<span class=\"mdLeft mdOverflowEllipsis\" style=\"width:100px;\">" + item.name + "</span>");
            html.Append('</a></li>');
        }
    }
    html.Append('<li><a href="' + md.api.options.host + 'group_all">所有群组</a></li>');
    html.Append('<li><a href="' + md.api.options.host + 'group/create" class="mdThemeColor3" >+ 创建新群组</a></li>');
    html.Append('</ul>');
    html.Append('</div>');

    html.Append('<div class="menuItem activeItem">');
    html.Append('<span class="mdLeft">任务协作</span><span class="mdLeft mdHidden arrowContainer"><i class="mLeft10 icon-arrow-up-border Left"></i></span><span class="Clear"></span>');
    html.Append('</div>');
    html.Append('<div class="menuItemLine"><div class="lineLeft"></div><div class="lineRight"></div><div class="mdClear"></div></div>');
    html.Append('<div class="menuItem_Content menuItem_ContentList">');
    html.Append('<ul class="navUL">');
    html.Append('<li class="navItem"><i class="mdThemeColor4 icon-task"></i><a href="' + md.api.options.host + 'apps/taskcenter/">任务中心</a></li>');
    html.Append('<li class="navItem"><i class="mdThemeColor4 icon-calander"></i><a href="' + md.api.options.host + 'apps/calendar/home">日程中心</a></li>');
    html.Append('</ul>');
    html.Append('</div>');

    html.Append('<div class="menuItem activeItem">');
    html.Append('<span class="mdLeft">知识管理</span><span class="mdLeft mdHidden arrowContainer"><i class="mLeft10 icon-arrow-up-border Left"></i></span><span class="Clear"></span>');
    html.Append('</div>');
    html.Append('<div class="menuItemLine"><div class="lineLeft"></div><div class="lineRight"></div><div class="mdClear"></div></div>');
    html.Append('<div class="menuItem_Content menuItem_ContentList">');
    html.Append('<ul class="navUL">');
    ...
    html.Append('<div class="menuItem activeItem">');
    html.Append('<span class="mdLeft">我的应用</span><span class="mdLeft mdHidden arrowContainer"><i class="mLeft10 icon-arrow-down-border Left"></i></span><span class="Clear"></span>');
    html.Append('</div>');
    html.Append('<div class="menuItemLine"><div class="lineLeft"></div><div class="lineRight"></div><div class="mdClear"></div></div>');
    html.Append('<div class="menuItem_Content" style="display:none">');
    html.Append('<ul class="appList">');

    if (md.api.options.applications) {
        for (var i = 0; i < md.api.options.applications.length; i++) {
            var app = md.api.options.applications[i];
            var appSettingID = "appSettingID_" + app.id;
            html.Append('<li>');
            html.Append('<div>');
            html.Append('<table width="100%" class="menuItem" ');
            if (app.is_admin=="1")
                html.Append(' onmouseover="$(\'#' + appSettingID + '\').show()" onmouseout="$(\'#' + appSettingID + '\').hide()"');
            html.Append('>');
            html.Append('<tr>');
            html.Append('<td>');

            html.Append('<div class="mdLeft"><a target="_blank" href="' + app.home_url + '" class="mdLeft mdAppName"><span class="mdLeft"><button type="button" class="mdImgButton"><img width="16" src="' + app.logo + '" style="display:inline-block" /></button></span><span class="mdOverflowEllipsis" style="width:65px" title="' + app.name + '">' + app.name + '</span></a></div>');
            html.Append('<div class="Clear"></div>');
            html.Append('</td>');
            html.Append('<td>');
            html.Append('<a id="' + appSettingID + '" target="_blank" title="应用管理 - ' + app.name + '" href="' + app.setting_url + '" class=" icon-tool" style="display:none;"></a>');
            html.Append('<td>');
            html.Append('</tr>');
            html.Append('</table>');
            html.Append('</div>');
    ...
    html.Append('<div class="menuItem_Content">');
    html.Append('<ul class="navUL">');
    html.Append('<li class="navItem"><i class="mdFont16 mdThemeColor4 icon-addapplication"></i><a href="https://app.mingdao.com">添加更多应用...</a></li>');
    html.Append('</ul>');
    html.Append('</div>');
    ...
    $("#mdMyGroups").die().live("click", function () {
        var oldI = $(this).find("span");
        var clone = $(this).find("span").clone(true);
        if ($("#mdUlGroups").is(":visible")) {
            clone.addClass("icon-arrow-down-border").removeClass("icon-arrow-up-border");
            $("#mdUlGroups").slideUp();
        }
        else {
            clone.addClass("icon-arrow-up-border").removeClass("icon-arrow-down-border");
            $("#mdUlGroups").slideDown();
        }
        oldI.after(clone);
        oldI.remove();
    });

    $("#mdLeftNav .activeItem").die().live("mouseover", function () {
        if (!$(this).find(".arrowContainer").is(":animated")) {
            $(this).find(".arrowContainer").fadeIn();
        }
    }).live("mouseout", function () {
        $(this).find(".arrowContainer").fadeOut();
    }).live("click", function () {
        var oldI = $(this).find("i");
        var colne = $(this).find("i").clone(true);
        if ($(this).next().next().is(":hidden")) {
            $(this).next().next().slideDown();
            colne.removeClass("icon-arrow-down-border").addClass("icon-arrow-up-border");
        } else {
            $(this).next().next().slideUp();
            colne.removeClass("icon-arrow-up-border").addClass("icon-arrow-down-border");
        }
        oldI.after(colne);
        oldI.remove();
    });
    ...
    $("#mdLeftNav .menuItem_Content ul li.navItem").die().live("mouseover", function () {
        $(this).addClass("mdThemeBGColor6").addClass("mdThemeBorderColor4").children().addClass("mdThemeColor3");
        $(this).find("i").removeClass("mdThemeColor4");
    }).live("mouseout", function () {
        $(this).removeClass("mdThemeBGColor6").removeClass("mdThemeBorderColor4").children().removeClass("mdThemeColor3");
        $(this).find("i").addClass("mdThemeColor4");
    });
};

md.api.getFooter = function (container) {
    
    var html = new md.system.string.StringBuilder();
    html.Append('<div id="mdFooter" class="feedFoot pTop10" style="height: 90px">');
    html.Append('<div class="footerNav">');
    html.Append('<div class="mdLeft leftNav">');
    html.Append('<span style="margin-left: 2px;"><a href="http://www.mingdao.com/features.htm" target="_blank">明道简介 </a></span>');
    html.Append('<span><a href="http://www.mingdao.com/mdworld.htm" target="_blank">新手入门</a></span> ');
    html.Append('<span><a href="http://www.mingdao.com/upgrade.htm" target="_blank">版本定价</a></span>');
    html.Append('<span><a href="http://tg.mingdao.com" target="_blank">推广赚钱</a></span>');
    ...
    html.Append('<span><a id="linkManager" href="' + md.api.options.host + 'myaccount/users_1" target="_blank">联系本公司管理员</a></span>');
    html.Append('</div>');
    html.Append('<div class="mdClear"></div>');
    html.Append('</div>');
    html.Append('<div class="mdDesc">');
    html.Append('<div class="mdNotice mdLeft">明道使用UCLOUD云计算服务，Geotrust加密传输和符合业界规范的运维保障。 </div>');
    html.Append('<div class="mdLeft"><a href="http://now.mingdao.com" class="mdThemeColor3" target="_blank">明道实时服务状态</a></div>');
    html.Append('<div class="mdRight mdThemeColor3">Designed in Shanghai, Assembled by YOU</div>');
    html.Append('<div class="mdClear"></div>');
    html.Append('</div>');
    html.Append('</div>');
